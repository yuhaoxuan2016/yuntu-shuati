      ...
      <div v-if="showUpdateLog" class="update-log">
        ...
        <div class="log-entry">
          ...
          <ul>
            ...
            <li>备份说明补一条同样的提醒；小程序端同步说明也写明「微信清除缓存会丢掉未上传的记录」</li>
          </ul>
        </div>
        <div class="log-entry">
          <span class="log-version">v1.2.51</span>
          <ul>
            <li>答完题后多了一块<b>「知识点总结（举一反三用）」</b>——默认折叠，点开是这一类题的通用规律（600~900 字），与针对本题的「解析」不是一回事；同样标<b>AI 生成，仅供参考</b></li>
            <li>题号旁显示<b>难度</b>（易/中/难），鼠标停上去是判定理由。这两个字段 4064 道题早就写进库里了，此前只有计算题的背题页能看到，练习、错题本、收藏、回顾都看不见</li>
          </ul>
        </div>
        <div class="log-entry">
          ...
          <ul>
            ...
            <li>手机端排版修复：首页「🌍 公共题库」标题此前被右侧长描述挤成「公」「共题」「库」三行（320~414px 均复现，桌面看不出来）；顶栏品牌名在 320px 宽机型上同样被挤成三行</li>
          </ul>
        </div>
        <div class="log-entry">
          ...
          <ul>
            ...
            <li>50 道题在答案旁提示「口径与现行规程有别」——<b>答案不改，判分仍以题库为准</b>，只是让你知道现行规程另有说法</li>
            <li>修复：答对的题目看不到解析（此前只有答错才显示，现在答完直接铺开）</li>
            <li>公共题库练习现在<b>记住进度</b>（只存在本机浏览器，不写入错题本与统计；导入本地后才有那些）</li>
            <li>已归档的题库沉底并默认折叠，可一键展开——含各 (旧) 库，以及用完归档的「变电运维教材」（它不是旧版，只是不再更新）</li>
            <li>归档状态改为<b>只看云端的归档标记</b>（不再靠题库名里的「(旧)」判断）：以后归档或取消归档只改数据，不用发版；卡片上也会显示「📦 已归档 · 不再更新」</li>
            <li>「添加到我的题库」的本地副本现在能看到解析／知识点／难度／配图；新增「从公共题库更新」</li>
          </ul>
        </div>
        <div class="log-entry">
          <span class="log-version">v1.2.48</span>
          <ul>
            <li>修复若干练习与考试中的判分问题（含未作答被算答错、判断题判分口径不一致）</li>
            <li>云同步修复：题库/题目修改能正常同步；删除题库/题目后不再被下次同步「复活」；同步结果如实提示</li>
            <li>导入修复：识别失败会明确提示；识别题数异常偏少时给出提醒</li>
            <li>联系与分享：二维码可正常扫码、反馈入口更可靠、分享卡片更新</li>
            <li>稳定性与界面细节改进</li>
          </ul>
        </div>
        <div class="log-entry">
